...
class Drivers_Mysql extends Drivers_Driver
{
    ...
    protected function compile_column($field_name, $params, $allow_order = false)
    {
        ...
        foreach ($params as $key => $param) {
            ...
            if (is_string($key)) {
                switch ($key) {
                    case 'after':   if ($allow_order) {
                        $order = "AFTER `$param`";
                    } break;
                    case 'null':    $null = (bool) $param; break;
                    case 'default':
                        if (is_string($param)) {
                            if (strtoupper($param) == 'NOW()') {
                                $default = 'DEFAULT NOW()';
                            } else {
                    ...
                    case 'update':
                        if (is_string($param)) {
                            if (strtoupper($param) == 'NOW()') {
                                $update = 'ON UPDATE NOW()';
                            } else {
                                $update = 'ON UPDATE ' . $this->db->escape($param);
                            }
                        } else {
                            $default = 'ON UPDATE ' . $param;
                        }
                        break;
